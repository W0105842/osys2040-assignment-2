...
async function getLikes() {
  try {
    const result = await PostgresUtil.pool.query('SELECT * FROM BandBlikes');
    return result.rows
  } catch(exception) {
...
async function addLike(handle, messageId) {
  try {
    const result = await PostgresUtil.pool.query(
      'INSERT INTO BandBlikes (created_by, for_message) VALUES ($1::text, $2::int);', [handle, messageId])
    return result
  } catch (exception) {
    if (exception.code === '42P01') { // Table is missing so we'll create it and try again
...
async function unLike(handle, messageId) {
  try {
    const result = await PostgresUtil.pool.query(
      'DELETE FROM BandBlikes WHERE created_by=$1::text AND for_message=$2::int;', [handle, messageId])
    return result
  } catch (exception) {
    if (exception.code === '42P01') { // Table is missing so we'll create it and try again
      await createMessageTable()
...
async function createLikeTable() {
  return await PostgresUtil.pool.query(`CREATE TABLE BandBlikes (
